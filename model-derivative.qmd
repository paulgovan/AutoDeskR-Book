...
%%| fig-width: 3.6
flowchart TB
  A[Upload file<br>uploadFile] --> B[Translate<br>translateObj / translateSvf]
  B --> C{checkFile}
  C -- pending / inprogress --> C
  C -- success --> D[Get output URN<br>getOutputUrn]
  D --> E[Download<br>downloadFile]
```

## Translate to OBJ

First get a token with `data:read` and `data:write` scopes. Note that only certain source formats can produce OBJ output (see the warning below) (DWG, IPT, IAM, IPN, F3D, FBX. See the [APS supported formats list](https://aps.autodesk.com/en/docs/model-derivative/v2/developers_guide/supported-translations/) for the full matrix).

```{r}
#| eval: !expr nzchar(Sys.getenv("client_id"))
resp <- getToken(
...
::: callout-warning
Tokens expire after 3600 seconds. A `401 Unauthorized` mid-session means the token timed out — re-call `getToken()`. See [Authentication](authentication.qmd#get-an-access-token) for details.
:::

APS requires the file's URN to be Base-64 encoded before it'll accept it — `jsonlite::base64_enc()` [@R-jsonlite] handles that:

```{r}
#| eval: !expr nzchar(Sys.getenv("client_id"))
myEncodedUrn <- jsonlite::base64_enc(myUrn)
myEncodedUrn
#> [1] "dXJuOmFkc2sub2JqZWN0cz...LmR3Zw=="
```
...
```{r}
#| eval: !expr nzchar(Sys.getenv("client_id"))
resp <- getOutputUrn(urn = myUrn, token = myToken)
