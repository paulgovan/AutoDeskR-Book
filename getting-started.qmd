...
flowchart TD
  ...
  B --> B1[Layer counts & attributes\nLayer Analysis · Attribute Extraction]
  B --> B2[Convert to PDF\nDesign Automation]
  B --> B3[3D mesh for analysis\nModel Derivative → OBJ]

  C --> C1[Embed 3D viewer\nViewer]
  C --> C2[Extract BIM metadata\nModel Derivative → SVF]
  C --> C3[Digital twin\nTandem Overview]

  D --> D1[3D mesh\nReality Capture → OBJ]
  D --> D2[Point cloud\nReality Capture → RCS → Point Cloud Analysis]
```
