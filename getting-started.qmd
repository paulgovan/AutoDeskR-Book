...
flowchart TD
  ...
  C --> D[uploadFile]
  D --> E{Use case}
  E --> F[translateObj /<br>translateSvf]
  E --> G[makePdf]
  E --> H[createPhotoscene]
  F --> I[viewer3D]
```

Not sure which API to use? The diagram below maps your input file type and goal to the right chapter:

```{mermaid}
%%| fig-width: 4.5
flowchart TD
  A{What do you<br>have?} --> B[DWG / DXF]
  A --> C[RVT / IFC / other BIM]
  A --> D[Photos / drone imagery]

  B --> B1[Layer counts & attributes<br>Layer Analysis · Attribute Extraction]
  B --> B2[Convert to PDF<br>Design Automation]
  B --> B3[3D mesh for analysis<br>Model Derivative → OBJ]

  C --> C1[Embed 3D viewer<br>Viewer]
  C --> C2[Extract BIM metadata<br>Model Derivative → SVF]
  C --> C3[Digital twin<br>Tandem Overview]

  D --> D1[3D mesh<br>Reality Capture → OBJ]
  D --> D2[Point cloud<br>Reality Capture → RCS → Point Cloud Analysis]
```
...
#> [1] "eyJhbGciOiJSUzI1NiIsImtpZCI6...cifQ..."
...
#> 1  file.dwg  urn:adsk.objects:...:file.dwg  3145728
```
