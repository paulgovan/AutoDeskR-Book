...
::: callout-important
**Replace before running:** swap `YOUR_CLIENT_ID` and `YOUR_CLIENT_SECRET` for the
Client ID and Secret of your own APS app (create one at
[aps.autodesk.com](https://aps.autodesk.com)). Every example in this book reads
these two values from `~/.Renviron`.
:::

::: callout-note
Store only long-lived credentials — `client_id` and `client_secret` — in `.Renviron`. Tokens expire after 3600 seconds and must be obtained fresh at the start of each session via `getToken()`. Never store a token in a static file.
:::
...
myToken
#> [1] "eyJhbGciOiJSUzI1NiIsImtpZCI6...Qssw5c"
```
...
str(resp$content)
#> List of 3
#>  $ access_token: chr "eyJhbGciOiJSUzI1Ni...w5c"
