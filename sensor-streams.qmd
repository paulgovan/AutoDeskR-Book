...
with your own Tandem facility URN, and set `client_id`/`client_secret` in
`~/.Renviron`. The stream IDs above are illustrative.
:::
...
::: callout-note
**Representative result.** This chart and the multi-stream overview below are
drawn from synthetic readings so they render without a live Tandem connection. The
same plotting code produces the same charts from your facility's real streams.
:::
...
all_readings <- data.frame(
  ...
  display_name = rep(c("HVAC Temp L2", "CO\u2082 Office 3",
                       "Lobby Occupancy", "Main Energy"), each = 96),
