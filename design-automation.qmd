...
::: callout-note
**Other engines are supported but not yet wrapped.** APS Design Automation also runs Revit, 3ds Max, Inventor, and Fusion 360 scripts via a generalised `submitWorkItem()` / `listEngines()` pattern. AutoDeskR currently exposes `makePdf()` as a convenience wrapper; support for the other engines is on the roadmap. See [github.com/paulgovan/AutoDeskR](https://github.com/paulgovan/AutoDeskR) to track progress or request prioritisation.
:::

```{mermaid}
%%| fig-width: 3.6
flowchart TB
  A[DWG file<br>at source URL] --> B[makePdf<br>submit work item]
  B --> C{checkPdf}
  C -- pending / inprogress --> C
  C -- success --> D[PDF file<br>at destination URL]
```
...
```{r}
#| eval: !expr nzchar(Sys.getenv("client_id"))
mySource      <- "http://download.autodesk.com/us/samplefiles/acad/
visualization_-_aerial.dwg"
myDestination <- "https://drive.google.com/folderview?
id=0BygncDVHf60mTDZVNDltLThLNmM&usp=sharing"
