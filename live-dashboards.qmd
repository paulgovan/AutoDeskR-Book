...
get_stream_readings <- function(token, facility_id, stream_id, hours_back = 24) {
  end_time   <- Sys.time()
...
server <- function(input, output, session) {
  ...
  readings <- reactive({
    ...
    get_stream_readings(token       = myToken,
...
::: callout-tip
The access token retrieved at startup expires after **1 hour**. For a long-running production deployment, move `getToken()` inside `get_stream_readings()` so a fresh token is requested on every poll rather than once at startup.
:::
