...
Got a drawing set? Multiple revisions? Discipline packages from different consultants? This chapter shows how to compare their layer structures systematically, what layers were added, what was removed, and where the object counts changed most between versions. Analysis uses `dplyr` [@R-dplyr] and `ggplot2` [@R-ggplot2].
...
@fig-layer-heatmap shows presence/absence across all drawings at a glance — blue tiles are present, grey tiles are absent.
...
#| fig-cap: "Layer presence matrix across drawing revisions. Blue = present, grey = absent."
...
@fig-layer-waterfall makes the direction and magnitude of each change immediately readable — green bars are additions, red bars are removals.
...
```{r}
#| label: fig-layer-waterfall
#| fig-cap: "Object count change per layer from v1 to v2. Green = added, red = removed."
ggplot(comparison,
